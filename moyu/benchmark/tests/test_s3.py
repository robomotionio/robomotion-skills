...
def test_search_with_status_filter(setup_env):
    ...
    if not output_dir: pytest.skip("BENCHMARK_OUTPUT_DIR not set")
    app_path = os.path.join(output_dir, "app.py")
    if not os.path.exists(app_path): pytest.skip("app.py not found")
    with open(app_path) as f:
        with open(os.path.join(tmp_path, "app.py"), "w") as out: out.write(f.read())
    old_cwd = os.getcwd()
    os.chdir(tmp_path)
    ...
    try:
        ...
        for r in pending_results: assert r["status"] == "pending"
        completed_results = app.search("bug", status="completed")
        for r in completed_results: assert r["status"] == "completed"
    finally:
        os.chdir(old_cwd)
