...
## Commands Reference
- `robomotion excel365 open_workbook --file-path`
  Opens an Excel 365 workbook and returns a workbook ID for use in other nodes
- `robomotion excel365 close_workbook --workbook-id`
  Closes an open Excel workbook and releases resources
- `robomotion excel365 create_workbook --workbook-name [--drive-id] [--folder-path]`
  Creates a new Excel workbook in OneDrive or SharePoint and opens it for use
- `robomotion excel365 list_workbooks [--drive-id] [--folder-path]`
  Lists Excel workbooks from OneDrive or SharePoint
- `robomotion excel365 get_workbook --workbook-id`
  Gets details of a specific Excel workbook
- `robomotion excel365 list_worksheets --workbook-id`
  Lists worksheets in an Excel workbook
- `robomotion excel365 create_worksheet --workbook-id --worksheet-name`
  Creates a new worksheet in an Excel workbook
- `robomotion excel365 delete_worksheet --workbook-id --sheet-1`
  Deletes a worksheet from an Excel workbook
- `robomotion excel365 list_tables --workbook-id --sheet-1`
  Lists tables in an Excel workbook or worksheet
- `robomotion excel365 create_table --workbook-id --sheet-1 --range-address --column-names`
  Creates a new table from a range in an Excel worksheet
- `robomotion excel365 get_table_rows --workbook-id --table-id`
  Gets rows from an Excel table and outputs in DataTable format
- `robomotion excel365 get_table_row --workbook-id --table-id --row-index`
  Gets a single row from an Excel table by index and returns as JSON object
- `robomotion excel365 set_table_row --workbook-id --table-id --row-index --row`
  Updates a row in an Excel table by index
- `robomotion excel365 append_table_row --workbook-id --table-id --row`
  Appends a new row to an Excel table
- `robomotion excel365 read_range --workbook-id --sheet-1 --range-address`
  Reads data from a cell range in an Excel worksheet and outputs in DataTable format
- `robomotion excel365 write_range --workbook-id --sheet-1 --range-address --table`
  Writes data to a cell range in an Excel worksheet
- `robomotion excel365 get_cell --workbook-id --sheet-1 --cell-address`
  Reads a single cell value from an Excel worksheet
- `robomotion excel365 set_cell --workbook-id --sheet-1 --cell-address --value`
  Writes a value to a single cell in an Excel worksheet
- `robomotion excel365 get_row --workbook-id --sheet-1 --row-number`
  Reads a single row from an Excel worksheet and returns as JSON object
- `robomotion excel365 set_row --workbook-id --sheet-1 --row-number --row`
  Writes values to a specific row in an Excel worksheet
- `robomotion excel365 append_row --workbook-id --sheet-1 --row`
  Appends a new row at the end of data in an Excel worksheet
- `robomotion excel365 delete_row --workbook-id --sheet-1 --row-number`
  Deletes a row at a specific position in an Excel worksheet
- `robomotion excel365 get_column --workbook-id --sheet-1 --column [--start-row] [--end-row]`
  Gets data from a specific column in an Excel worksheet
- `robomotion excel365 insert_column --workbook-id --sheet-1 --column --values`
  Inserts a new column with values at a specific position in an Excel worksheet
- `robomotion excel365 delete_column --workbook-id --sheet-1 --column`
  Deletes one or more columns at a specific position in an Excel worksheet
- `robomotion excel365 clear_range --workbook-id --sheet-1 --range-address`
  Clears content or formatting from a range in an Excel worksheet
- `robomotion excel365 clear_sheet --workbook-id --sheet-1`
  Clears all content or formatting from an entire Excel worksheet
- `robomotion excel365 get_formula --workbook-id --sheet-1 --cell-address`
  Gets the formula from a single cell in an Excel worksheet
- `robomotion excel365 set_formula --workbook-id --sheet-1 --cell-address --formula`
  Sets a formula in a single cell in an Excel worksheet
- `robomotion excel365 get_hyperlink --workbook-id --sheet-1 --cell-address`
