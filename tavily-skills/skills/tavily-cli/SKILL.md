...
`tvly` is already installed, at a pinned version, with the Tavily skills
you are reading. Do not reinstall it, run `tvly update`, or run `tvly init`:
each one replaces the reviewed CLI or skills with whatever is newest.

Authentication comes from a `TAVILY_API_KEY` set in the agent's environment.
The agent runs unattended: if a command needs a key and none is set, report
that and stop instead of starting `tvly login`.
